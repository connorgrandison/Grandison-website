...
Published to Cloudflare on 24 September 2026 as `grandison-plumbing-heating` in Connor's account. Main site: https://grandisonplumbingandheating.co.uk . The separate workers.dev address is disabled; share the main business domain. The confirmed source repository is https://github.com/connorgrandison/Grandison-website on `main`. Push verified source and tracked output there for each update. GitHub pushes do not currently deploy automatically: publish to Cloudflare separately and verify both outcomes. The earlier Sites URL is a separate, older preview and should no longer be shared.
...
Photo-preview metadata was published to the main Cloudflare site on 24 September 2026. Live checks confirmed that the homepage, heating, bathrooms and freestanding-bath project page expose an absolute HTTPS `og:image` URL, a large-image card, and a JPEG that returns HTTP 200. All 36 generated pages passed local metadata and image-availability checks. The homepage/general-page default is the supplied freestanding-bath photograph; service and project pages use their matching supplied image where available. `src/social.mjs` selects the image. Apps may cache previous previews or crop the photographs differently; final appearance inside every messaging app has not been independently tested.
